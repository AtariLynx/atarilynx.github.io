---
title: RSA encryption for Atari Lynx
abstract: How RSA encryption is used to sign and secure the Atari Lynx boot loader
---
...
The work for these steps was performed at two stages by different people using different tools. The first step is done by the Lynx developer who is creating a Lynx program or game.
...
The title screen file must contain a 32-byte set of colors and a SCB definition for a sprite. The 32-byte color palette is copied directly into the registers `$FDA0` to `$FDBF`. The SCB information is loaded and used to display the boot screen. Typically the first sprite clears the entire screen and is chained with the rest of the sprite or sprites.
...
The various aspects of the cartridge found in the cartridge definition are as follows:
...
The `HandyROM` tool uses `.hsf` files to create a binary file with the `.rom` extension that has a reserved space of required bytes (filled with zeros `0x00`) for the encrypted header. The header with the encrypted boot loader will be created in the next step and is going to replace the placeholder zeros.
...
A number of new files are created. 
