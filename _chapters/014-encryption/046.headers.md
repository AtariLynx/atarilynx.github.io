...
The BLL tool chain includes an executable `lynxenc` that can encrypt a binary file. It will make some decisions on the size of the data and write a file containing the encrypted version. The input for`lynxenc` are the input and output file names:
...
// TODO:
Insert pictures for encryption process, screenshots of tooling
