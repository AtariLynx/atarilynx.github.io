...
Additionally, since the 6502 interrupt sequence does not store the `Y` register, you will need to store and restore `Y`` in your handler if you plan to not loose the original value before the ISR. In most cases it is not necessary to do this.
...
Similarly, `SETIRQ` also sets the interrupt handler, but in addition will enable the interrupt. It is safe to call whenever other interrupts are already active, as the macro will start with disabling interrupts using `SEI`.
