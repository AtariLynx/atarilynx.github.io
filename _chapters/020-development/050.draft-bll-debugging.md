...
The bytes sent to the Lynx will always be echoed to the debugger, as the `RX` and `TX` line are connected. The table above shows the bytes that are actually sent from the Lynx by the `BRKserver` routine. When listening over the ComLynx serial connection these bytes are preceded by the bytes sent. 
